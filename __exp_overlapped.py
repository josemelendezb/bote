...
def get_overlapped_senteces(dataset, cluster, overlapped_type):
    ...
    for k, (s, t) in enumerate(zip(data[0], data[1])):
        ...
        if overlapped_type ==  'ap' and aspect_overlapped:
            sentences_v.append(s)
            triplets_v.append(triplets)
        elif overlapped_type ==  'op' and opinion_overlapped:
            sentences_v.append(s)
            triplets_v.append(triplets)
        elif overlapped_type ==  'ap_op' and opinion_aspect_overlapped:
            sentences_v.append(s)
            triplets_v.append(triplets)
        elif overlapped_type ==  'not' and not aspect_overlapped and not opinion_overlapped:
            sentences_v.append(s)
            triplets_v.append(triplets)

    print(sentences_v)
    print(triplets_v)
    return sentences_v, triplets_v
...
if __name__ == '__main__':
    ...
    parser.add_argument('--overlapped_type', default='ap', type=str)
    opt = parser.parse_args()

    print(opt.overlapped_type)
    ...
    for dataset in datasets:
        ...
        texts, triplets_real = get_overlapped_senteces(dataset, cluster, opt.overlapped_type)
        all_outputs = []
        all_targets = []
        count_triplets = 0
